...
async def list_delegation_runs(request: Request, parent_agent_id: Optional[str] = None) -> List[DelegationRunSummary]:
    registry = request.app.state.registry
    runs = registry.subagent_registry.list_runs(parent_agent_id=parent_agent_id)
    return [
        DelegationRunSummary(
            run_id=run.run_id,
            parent=run.parent_agent_id,
            child=run.child_agent_id,
            status=run.status.value,
            task=run.task[:100],
        )
        for run in runs
