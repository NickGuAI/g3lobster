...
class MemorySearchEngine:
    ...
    def _sort_key(hit: MemorySearchHit) -> float:
        ...
        if hit.timestamp:
            ...
            try:
                ...
                pass
        # Apply salience weight for journal hits.
    ...
    def _search_journal_file(
        ...
        *,
        ...
        """Search a JSONL journal file, weighting results by salience."""
        if not path.exists() or not path.is_file():
            return []
        ...
        for line_number, raw in enumerate(lines, start=1):
            ...
            timestamp = entry.timestamp or (
                datetime.combine(file_date, datetime.min.time(), tzinfo=timezone.utc).isoformat()
                if file_date
                else None
            )
            hit = MemorySearchHit(
            ...
            # Attach salience weight for ranking.
            hit._salience_weight = entry.salience.search_weight  # type: ignore[attr-defined]
            hits.append(hit)
